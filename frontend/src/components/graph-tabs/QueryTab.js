...
const AIQL_CHEATSHEET = [
  ...
  { cmd: 'FIND nodes', desc: 'Find all nodes' },
  { cmd: 'FIND edges', desc: 'Find all edges' },
];
...
function GraphPanel({ nodes, edges }) {
  ...
  return (
    <div className="flex" style={{ height: 360 }}>
      ...
      <div className="w-44 overflow-y-auto p-2" style={{ borderLeft: '1px solid var(--neo-border)', background: 'var(--neo-surface)' }}>
        <div className="mb-2">
            ...
            <div className="space-y-0.5">
              {Object.entries(selectedItem).filter(([k]) => !SKIP.has(k) && !k.startsWith('_')).map(([k, v]) => (
                <div key={k} className="flex justify-between text-xs gap-1">
                  ...
                  <span className="text-right truncate" style={{ color: 'var(--neo-text)', maxWidth: 90 }}>{typeof v === 'object' ? JSON.stringify(v) : String(v)}</span>
                </div>
              ))}
            </div>
          </>
...
export default function QueryTab({ graphName }) {
  ...
  const handleRun = async () => {
    ...
    try {
      ...
      if ((res.data.nodes?.length || 0) > 0) setViewMode('graph');
    } catch (err) {
      const detail = err.response?.data?.detail;
      const msg = typeof detail === 'string' ? detail : detail ? JSON.stringify(detail) : 'Query execution failed';
  ...
  return (
    <div className="h-full flex gap-4">
      ...
      <div className="flex-1 space-y-4 overflow-auto">
        ...
        <div className="rounded-xl overflow-hidden" style={{ background: 'var(--neo-surface)', border: '1px solid var(--neo-border)' }}>
          <div className="flex items-center justify-between px-4 py-2 border-b" style={{ borderColor: 'var(--neo-border)' }}>
            <span className="text-xs" style={{ color: 'var(--neo-text-muted)' }}>AIQL</span>
            <div className="flex items-center gap-1.5">
              <button onClick={() => setShowCheatsheet(!showCheatsheet)} className="p-1.5 rounded transition hover:opacity-70" style={{ color: 'var(--neo-text-muted)' }} title="AIQL Reference"><BookOpen size={14} /></button>
              <button onClick={() => setShowSaved(!showSaved)} className="p-1.5 rounded transition hover:opacity-70" style={{ color: 'var(--neo-text-muted)' }} title={`Saved (${savedQueries.length})`}><Save size={14} /></button>
              <button onClick={() => { if (query.trim()) setShowSaveInput(true); }} className="p-1.5 rounded transition hover:opacity-70" style={{ color: 'var(--neo-text-muted)' }} title="Save query"><Download size={14} /></button>
          ...
          <textarea value={query} onChange={(e) => setQuery(e.target.value)} onKeyDown={handleKeyDown} placeholder="Enter AIQL query... (Ctrl+Enter to run)" rows={5} className="w-full px-4 py-3 text-sm outline-none resize-none" style={{ background: 'var(--neo-bg)', color: 'var(--neo-text)', fontFamily: "'JetBrains Mono', 'Fira Code', monospace" }} />
        </div>
          ...
          <div className="rounded-xl overflow-hidden" style={{ background: 'var(--neo-surface)', border: '1px solid var(--neo-border)' }}>
            <div className="flex items-center justify-between px-4 py-2 border-b" style={{ borderColor: 'var(--neo-border)' }}>
              <div className="flex items-center gap-2">
                ...
                {results.duration_ms != null && <span className="text-xs" style={{ color: 'var(--neo-text-muted)' }}>({results.duration_ms}ms)</span>}
                {results.data && Array.isArray(results.data) && <span className="text-xs" style={{ color: 'var(--neo-text-muted)' }}>({results.data.length} rows)</span>}
              </div>
              <div className="flex items-center gap-1">
                {['table', 'json', ...(hasGraphData ? ['graph'] : [])].map((mode) => (
                  <button key={mode} onClick={() => setViewMode(mode)} className="px-2 py-1 rounded text-xs font-medium" style={{ background: viewMode === mode ? 'var(--neo-bg)' : 'transparent', color: viewMode === mode ? 'var(--neo-text)' : 'var(--neo-text-muted)', border: viewMode === mode ? '1px solid var(--neo-border)' : '1px solid transparent' }}>
            ...
            {viewMode === 'graph' && hasGraphData && <GraphPanel nodes={results.nodes} edges={results.edges} />}

            {viewMode !== 'graph' && (
              <div className="max-h-80 overflow-auto">
                {results.error ? (
                  <div className="p-4 text-sm" style={{ color: '#ef4444' }}>{typeof results.error === 'string' ? results.error : JSON.stringify(results.error)}</div>
                ) : viewMode === 'json' ? (
                  <pre className="p-4 text-xs whitespace-pre-wrap" style={{ color: 'var(--neo-text)', fontFamily: "'JetBrains Mono', monospace" }}>{JSON.stringify(results.data || results, null, 2)}</pre>
                      ...
                      {results.data.map((row, i) => (
                        <tr key={i} style={{ borderBottom: '1px solid var(--neo-border)' }}>
                          {Object.values(row).map((val, j) => (
                            <td key={j} className="px-3 py-2" style={{ color: 'var(--neo-text)' }}>{typeof val === 'object' ? JSON.stringify(val) : String(val ?? '')}</td>
                          ))}
                        </tr>
                      ))}
                    </tbody>
          ...
          <div>
            ...
            <div className="space-y-1">
              {history.slice(0, 8).map((h, i) => (
                <button key={i} onClick={() => setQuery(h.query)} className="block w-full text-left px-3 py-1.5 rounded-lg text-xs truncate transition hover:opacity-80" style={{ background: 'var(--neo-surface)', border: '1px solid var(--neo-border)', color: 'var(--neo-text)', fontFamily: "'JetBrains Mono', monospace" }}>
                  {h.query}
                </button>
              ))}
            </div>
      ...
      {(showSaved || showCheatsheet) && (
        <div className="w-60 shrink-0 space-y-4 overflow-auto">
          {showSaved && (
            <div className="rounded-xl p-3" style={{ background: 'var(--neo-surface)', border: '1px solid var(--neo-border)' }}>
              <h3 className="text-sm font-semibold mb-2" style={{ color: 'var(--neo-text)' }}>Saved Queries</h3>
          ...
          {showCheatsheet && (
            <div className="rounded-xl p-3" style={{ background: 'var(--neo-surface)', border: '1px solid var(--neo-border)' }}>
              <h3 className="text-sm font-semibold mb-2" style={{ color: 'var(--neo-text)' }}>AIQL Reference</h3>
              <div className="space-y-2">

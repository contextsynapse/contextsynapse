"""AIContextDB Agent Wrapper — zero-config connection for any AI agent.
...
Examples::
    ...
    agent = AIContextDBAgent("my-bot", server="https://abc.ngrok-free.app")
...
_CREDS_DIR = Path.home() / ".contextcore"


class AIContextDBAgent:
    """Quickstart wrapper for connecting any agent to AIContextDB.

    On first use, registers with the server and saves credentials to
    ``~/.contextcore/<agent_name>.json``. On subsequent uses, loads saved
    credentials automatically.

    Args:
        name: Agent name (used for registration and credential lookup).
        server: AIContextDB server URL (e.g. ngrok public URL).
        capabilities: Agent capabilities. Default: ["read", "write"].
        session: Session name to auto-join/create. Default: "shared".
        creds_dir: Directory for credential files. Default: ~/.contextcore/
    """
    ...
    def __init__(
        ...
        # Connect
        self.client = AIContextDB(base_url=self.server, api_key=self._api_key)
    ...
    def _register(self):
        tmp = AIContextDB(base_url=self.server)
    ...
    def __repr__(self) -> str:
        return f"AIContextDBAgent(name={self.name!r}, server={self.server!r})"
